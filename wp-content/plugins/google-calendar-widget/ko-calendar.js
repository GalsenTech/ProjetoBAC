...
var ko_calendar = function ()
{
	...
	function buildLocation(entry)
	{
		...
		{
			var locationTitleTag = document.createElement('u');
			locationTitleTag.appendChild(document.createTextNode("Local"));
			locationDiv.appendChild(locationTitleTag);
	...
	function formatEventDetails(titleFormat, event)
	{
		...
		if (startDateTime)
		{
			...
			{
				var startHour = startDateTime.getDate().toString("HH") + "h";
				var startMinutes = startDateTime.getDate().toString("mm");
				var startMinutesInt = parseInt(startMinutes);
				if(startMinutes && startMinutesInt > 0) startMinutes += "min";
				else startMinutes = "";
				
				startTimeString = startHour + startMinutes; //startDateTime.getDate().toString("h:mm tt");
				if (endDateTime)
				{
					var endHour = endDateTime.getDate().toString("HH") + "h";
					var endMinutes = endDateTime.getDate().toString("mm");
					var endMinutesInt = parseInt(endMinutes);
					if(endMinutes && endMinutesInt > 0) endMinutes += "min";
					else endMinutes = "";
					
					endTimeString = endHour + endMinutes;  //endDateTime.getDate().toString("h:mm tt");
				}
			}
		}
	...
	function createClickHandler(item, entry)
	{
		...
		return function () 
		{
			if (descDiv == null)
			{
				...
				bodyDiv.innerHTML = Wiky.toHtml(entry.description != null ? "<b>Descrição: </b>" + entry.description : "");
	...
	function createListEvents(titleId, outputId, maxResults, autoExpand, googleService, calendars, titleFormat)
	{
		...
		function processFinalFeed(feedRoot) {
			...
			for (var i = 0; i < len; i++) {
				...
				dateString = startJSDate.toString('dd.MM.yy');
				...
				*/
				{				
					...
					/*-------------------------------------------------*/
					/*Inclui conteúdo por padrão, sem controle de click*/
					/*-------------------------------------------------*/
					descDiv = document.createElement('div');
					...
					bodyDiv.setAttribute('className','ko-calendar-entry-body');
					bodyDiv.setAttribute('class','ko-calendar-entry-body');
					bodyDiv.innerHTML = Wiky.toHtml(entry.description != null ? "<u>Descrição</u>: " + entry.description : "");
					descDiv.appendChild(bodyDiv);
